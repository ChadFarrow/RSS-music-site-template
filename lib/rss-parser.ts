...
export interface RSSAlbum {
  ...
  feedUrl?: string;
  lastUpdated?: string;
  paymentRecipients?: Array<{ address: string; split: number; name?: string; fee?: boolean; type?: string }>;
  // Podcast GUIDs for Nostr boost tagging
...
export class RSSParser {
  ...
  /**
   * Detect feed type by checking the channel's podcast:medium tag
   * Returns 'publisher' if podcast:medium="publisher" is found, otherwise 'album'
   */
  static async detectFeedType(feedUrl: string): Promise<'album' | 'publisher'> {
    try {
      ...
      if (!response.ok) {
        devLog(`⚠️ Failed to fetch feed for type detection: ${response.status}`);
        return 'album'; // Default to album on error
      }
      ...
      if (parserError) {
        devLog('⚠️ XML parsing error during type detection');
        return 'album'; // Default to album on parse error
      }
      
      // Extract channel info
      const channels = xmlDoc.getElementsByTagName('channel');
      if (!channels || channels.length === 0) {
        devLog('⚠️ No channel found during type detection');
        return 'album'; // Default to album if no channel
      }
      
      const channel = channels[0];
      
      // Check for podcast:medium tag at channel level
      const mediumElements = Array.from(channel.getElementsByTagName('podcast:medium'));
      for (const element of mediumElements) {
        const medium = (element as Element).textContent?.trim().toLowerCase();
        if (medium === 'publisher') {
          devLog(`✅ Detected publisher feed: ${feedUrl}`);
          return 'publisher';
        }
      }
      
      // Also check for itunes:category or other indicators if needed
      // For now, default to album
      devLog(`📀 Detected album feed: ${feedUrl}`);
      return 'album';
      
    } catch (error) {
      console.error('❌ Error detecting feed type:', error);
      // Default to album on any error
      return 'album';
    }
  }
  ...
  static async parseAlbumFeed(feedUrl: string): Promise<RSSAlbum | null> {
    ...
    return withRetry(async () => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        // Support both keysend and lnaddress methods, and both node and lnaddress recipient types
        let trackPaymentRecipients: Array<{ address: string; split: number; name?: string; fee?: boolean; type?: string }> | undefined;
        
        if (trackValue && trackValue.type === 'lightning' && (trackValue.method === 'keysend' || trackValue.method === 'lnaddress') && trackValue.recipients && trackValue.recipients.length > 0) {
          trackPaymentRecipients = trackValue.recipients
            ...
            .map(r => ({
              ...
              name: r.name,
              fee: r.fee,
              type: r.type // Include the type field for payment routing (node or lnaddress)
            }));
          
          if (trackPaymentRecipients.length > 0) {
            verboseLog(`💰 Processed ${trackPaymentRecipients.length} payment recipients for track "${trackTitle}" (method: ${trackValue.method})`);
          }
        }
      ...
      // Support both keysend and lnaddress methods, and both node and lnaddress recipient types
      let paymentRecipients: Array<{ address: string; split: number; name?: string; fee?: boolean; type?: string }> | undefined;
      
      if (value && value.type === 'lightning' && (value.method === 'keysend' || value.method === 'lnaddress') && value.recipients && value.recipients.length > 0) {
        ...
        if (paymentRecipients.length > 0) {
          verboseLog(`💰 Processed ${paymentRecipients.length} payment recipients for "${title}" (method: ${value.method})`);
        }
      }
    ...
    try {
      ...
      if (feedUrl === 'iroh-aggregated') {
        ...
        remoteItems.forEach((item: unknown) => {
          ...
          const feedGuid = element.getAttribute('feedGuid');
          const feedUrl = element.getAttribute('feedUrl');
          const feedImg = element.getAttribute('feedImg');
      ...
      // Check if channel has podcast:medium="publisher" (for validation/logging)
      const channelMediumElements = Array.from(channel.getElementsByTagName('podcast:medium'));
      let isPublisherFeed = false;
      for (const element of channelMediumElements) {
        const medium = (element as Element).textContent?.trim().toLowerCase();
        if (medium === 'publisher') {
          isPublisherFeed = true;
          devLog(`✅ Confirmed publisher feed (channel-level medium=publisher): ${feedUrl}`);
          break;
        }
      }
      ...
      devLog(`🔍 Found ${remoteItems.length} podcast:remoteItem elements in feed: ${feedUrl}`);
      ...
      if (publisherItems.length === 0 && remoteItems.length > 0) {
        devLog(`⚠️ No music items found in publisher feed, but ${remoteItems.length} remoteItems exist. Checking all mediums...`);
        // Log what mediums we found for debugging
        const mediumsFound = new Set<string>();
        remoteItems.forEach((item: unknown) => {
          const element = item as Element;
          const medium = element.getAttribute('medium');
          if (medium) mediumsFound.add(medium);
        });
        devLog(`📊 Found remoteItems with mediums: ${Array.from(mediumsFound).join(', ')}`);
      }
