...
export const createAlbumBackground = (colors: ExtractedColors): string => {
  ...
  // Preserve more of the original color, especially for bright colors
  const isVeryBright = hsl.l > 70; // Very bright colors (like yellow)
  const isLightColor = hsl.l > 60; // Light colors
  
  let gradient: string;
  
  if (isVeryBright) {
    // For very bright colors: minimal darkening to preserve vibrancy
    // Reduce lightness by only 5-15% instead of 25-40%
    const lightnessReduction1 = Math.max(5, hsl.l * 0.1); // ~10% reduction
    const lightnessReduction2 = Math.max(8, hsl.l * 0.12); // ~12% reduction
    const lightnessReduction3 = Math.max(10, hsl.l * 0.15); // ~15% reduction
    const lightnessReduction4 = Math.max(12, hsl.l * 0.18); // ~18% reduction
    
    gradient = `linear-gradient(135deg, 
      hsl(${hsl.h}, ${Math.min(100, hsl.s + 15)}%, ${Math.max(25, hsl.l - lightnessReduction1)}%) 0%, 
      hsl(${hsl.h}, ${Math.min(100, hsl.s + 10)}%, ${Math.max(22, hsl.l - lightnessReduction2)}%) 25%, 
      hsl(${(hsl.h + 10) % 360}, ${Math.max(40, hsl.s - 5)}%, ${Math.max(20, hsl.l - lightnessReduction3)}%) 75%, 
      hsl(${(hsl.h + 20) % 360}, ${Math.max(35, hsl.s - 10)}%, ${Math.max(18, hsl.l - lightnessReduction4)}%) 100%)`;
  } else if (isLightColor) {
    // For light colors: moderate darkening (10-20% instead of 25-40%)
    const lightnessReduction1 = Math.max(10, hsl.l * 0.15); // ~15% reduction
    const lightnessReduction2 = Math.max(12, hsl.l * 0.18); // ~18% reduction
    const lightnessReduction3 = Math.max(15, hsl.l * 0.20); // ~20% reduction
    const lightnessReduction4 = Math.max(18, hsl.l * 0.22); // ~22% reduction
    
    gradient = `linear-gradient(135deg, 
      hsl(${hsl.h}, ${Math.min(100, hsl.s + 15)}%, ${Math.max(20, hsl.l - lightnessReduction1)}%) 0%, 
      hsl(${hsl.h}, ${Math.min(100, hsl.s + 10)}%, ${Math.max(18, hsl.l - lightnessReduction2)}%) 25%, 
      hsl(${(hsl.h + 12) % 360}, ${Math.max(35, hsl.s - 8)}%, ${Math.max(15, hsl.l - lightnessReduction3)}%) 75%, 
      hsl(${(hsl.h + 25) % 360}, ${Math.max(30, hsl.s - 15)}%, ${Math.max(12, hsl.l - lightnessReduction4)}%) 100%)`;
  } else {
