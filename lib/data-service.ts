...
class DataService {
  ...
  async getPublisherData(publisherId: string): Promise<PublisherData | null> {
    ...
    const publisherIdLower = publisherId.toLowerCase();
    const publisherIdSlug = createSlug(publisherId);
    ...
    const publisherFeed = feeds.find(feed => {
      ...
      const idMatch = feed.id === publisherIdLower || feed.id.includes(publisherIdLower) || publisherIdLower.includes(feed.id);
      const idSlugMatch = createSlug(feed.id) === publisherIdSlug;
      const titleMatch = feed.title?.toLowerCase() === publisherIdLower;
      const titleSlugMatch = feed.title && createSlug(feed.title) === publisherIdSlug;
      const publisherTitleMatch = feed.parsedData?.publisherInfo?.title?.toLowerCase() === publisherIdLower;
      const publisherTitleSlugMatch = feed.parsedData?.publisherInfo?.title && createSlug(feed.parsedData.publisherInfo.title) === publisherIdSlug;
      const urlMatch = feed.originalUrl.includes(publisherId);
      const artistMatch = feed.parsedData?.publisherInfo?.artist?.toLowerCase().includes(publisherIdLower);
      const artistSlugMatch = feed.parsedData?.publisherInfo?.artist && createSlug(feed.parsedData.publisherInfo.artist) === publisherIdSlug;

      return idMatch || idSlugMatch || titleMatch || titleSlugMatch || publisherTitleMatch || publisherTitleSlugMatch || urlMatch || artistMatch || artistSlugMatch;
    });
