import { RSSParser, RSSAlbum, RSSPublisher } from './rss-parser';
...
export class FeedParser {
  ...
  static async parseAllFeeds(): Promise<FeedParseReport> {
    ...
    // Auto-detect feed types before parsing (for feeds that might be publisher feeds)
    console.log(`🔍 Auto-detecting feed types...`);
    await FeedManager.detectAndUpdateFeedTypes();
    
    // Reload feeds after type detection to get updated types
  ...
  static async parseFeedById(feedId: string): Promise<ParsedFeedData | null> {
    let feeds = FeedManager.getActiveFeeds();
    let feed = feeds.find(f => f.id === feedId);
    
    if (!feed) {
      throw new Error(`Feed with ID '${feedId}' not found`);
    }
    
    // Auto-detect feed type if it's still set to default 'album' type
    // This ensures publisher feeds are detected even if not detected during initial load
    if (feed.type === 'album') {
      console.log(`🔍 Auto-detecting type for feed: ${feed.originalUrl}`);
      const detectedType = await RSSParser.detectFeedType(feed.originalUrl);
      if (detectedType !== feed.type) {
        console.log(`✅ Detected ${detectedType} feed, updating feed type`);
        FeedManager.updateFeed(feedId, { type: detectedType });
        // Reload to get updated feed
        FeedManager.clearCache();
        feeds = FeedManager.getActiveFeeds();
        feed = feeds.find(f => f.id === feedId);
        if (!feed) {
          throw new Error(`Feed with ID '${feedId}' not found after type update`);
        }
      }
    }
